...
export default function CardPix({ totalpedido, produtosoncarrinho }) {
  ...
  const handleupdateestoques = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/post/upload/estoques`, {
        method: 'POST',
        headers: {
  ...
  const handlePucheInfosNotaFiscal = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/post/notasfiscais', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handlePurchaseSuccess = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/post/sucessocompra', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${process.env.API_KEY}`
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/criarpix', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
