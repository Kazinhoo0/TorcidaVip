...
export default function CardFavoriteProd({ infoprods }) {
    ...
    const { dadosuserlogon, setaddonfavorite, fetchProductDetails } = useContext(ContextProducts);
    ...
    const fetchRemoveItemFavorito = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/post/removeitemfavorito`, {
                method: 'POST',
    ...
    const handleOpenDetailsProd = () => {
        const id = infoprods.itemid
    ...
    return (
        ...
        <div onClick={handleOpenDetailsProd} className="favorite-item-container">
